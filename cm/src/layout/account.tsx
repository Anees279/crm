...
const ClientData: React.FC<ClientDataProps> = ({ editable }) => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await axios.get('https://crm-eight-pied.vercel.app/api/clients');
  ...
  const addRow = async () => {
    if (newRow.accountName && newRow.phone && newRow.website && newRow.accountOwner) {
      try {
        const response = await axios.post('https://crm-eight-pied.vercel.app/api/clients', newRow);
  ...
  const deleteRow = async (id: string) => {
    try {
      await axios.delete(`https://crm-eight-pied.vercel.app/api/clients/${id}`);
